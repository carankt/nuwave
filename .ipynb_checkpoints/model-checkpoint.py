...
def Conv1d(*args, **kwargs):
    ...
    nn.init.kaiming_normal_(layer.weight)  # same as He initialization
    return layer

class DiffusionEmbedding(nn.Module):
    def __init__(self, hparam):
        super().__init__()
        self.n_channels = hparam.ddpm.pos_emb_channels #128
        self.scale = hparam.ddpm.pos_emb_scale # 50k
        self.out_channels = hparam.arch.pos_emb_dim # 512
        half_dim = self.n_channels // 2 # 64
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, x_low, noise_level):
        ...
        gate, filter = torch.chunk(y, 2, dim=1) #spliting into 2 chunks
